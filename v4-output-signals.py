...
# split into samples
train_ts, train_value = u.split_sequence_v4(train_data[["Close"]].values, n_steps, signal_base_on_next_N_candles)

validate_ts, validate_value = u.split_sequence_v4(validation_data[["Close"]].values, n_steps,
...
def run_train(log_dir, checkpoints_dir, h_params):
    ...
    # output layer
    model.add(Dense(train_value.shape[1], activation='softmax'))
